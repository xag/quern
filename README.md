...
Every effort above exists because a machine could only act on meaning that had first been reduced to structure: a type, a schema, a relation declared in advance, every term defined before anybody used one. That reduction was the expense — and it was also what forced the centralizing, because a hundred people cannot each invent types in a system that has to stay consistent. Somebody had to own the vocabulary, so everybody else had to wait for it, so it had to be finished first.

A model needs none of that. It reads the sentence a practitioner would have written anyway — *something known-unsound, carried deliberately, with its cost stated rather than forgotten* — and acts on it: finds the nodes that are one, tells you which of them are unpaid, refuses to let one past a gate. Nothing was formalized. So semantics can be prose, and prose can be partial where the work is partial, provisional where the thinking is provisional, invented on the spot and sharpened the next time somebody reads it.
